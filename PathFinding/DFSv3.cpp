...
public:
	...
	pair<int, int> next_feasible_move(vector<vector<char>> &maze, pair<int, int> current_index)
	{
		if (combined_check(maze, current_index, DOWN))
	...
	stack<pair<int, int>> depth_first_search()
	{
		...
		while (maze[current_index.first][current_index.second] != 'E')
		{
			...
			{
				current_index = next_feasible_move(maze, current_index);
			...
			// while (ifNextMovePossible(maze, current_index))
			// {
			// 	current_index = next_feasible_move(maze, current_index);
			// 	visited[current_index] = true;
			// 	st.push(current_index);
